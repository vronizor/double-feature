...
| **The detail overlay is broken on a phone** | ⏳ ready | **Seen on a real phone, and the two-column desktop layout did not collapse.** The poster sits left and the info column right, clipped at the card's edge — the title runs under the close button, "The Criterion Collectio", the overview and "Where to watch" all lose their right-hand side, and the trailer lands at the bottom of the empty poster column with a screen-tall void above it. Already ruled out: the viewport meta is present and correct, and a collapse rule DOES exist at `max-width: 620px` setting one column and re-placing poster, info and trailer into it. So the question is why that rule did not take — a browser in desktop-site mode would explain every symptom at once, and so would anything re-asserting `.modal-info`'s `grid-column: 2` later in the cascade. **Establish which before changing CSS**, and note that a hidden automation tab does not reflow on resize, so it cannot answer this — see §1 |
